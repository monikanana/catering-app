...
class App extends Component {
    ...
    createOrder() {
        const {basket, price, customerId, locationId, date, weekdays} = this.state;
        ...
        if (weekdays.length === 0) {
            fetch(`http://localhost:8080/catering_app_war_exploded/customers/${customerId}/orders`, {
                method: "POST",
                headers: {
        ...
        return (
            <div className="App">
                ...
                <div className="content">
                    ...
                    <Typography.Title level={2} className="category-name">Najpopularniejsze Potrawy</Typography.Title>
                    <ul className="top-meals-list">
                        {topMeals && topMeals.map(meal => {
                            return <li key={meal.id} className="meal">{meal.name + " " + meal.price + "zł"}
                                {this.isAdded(meal.id) ?
                                    <Button className="meal-btn" onClick={() => this.removeMeal(meal)}
                                            type="danger">Usuń</Button> :
                                    <Button className="meal-btn" onClick={() => this.addMeal(meal)}
                                            type="primary">Dodaj</Button>}
                            </li>
                        })}
                    </ul>
                    <ul className="meals-categories-list">
                        {meals && meals.map(mealCategory => {
                            return (
                                <li key={mealCategory.categoryId} className="meals-category">
                                    <Typography.Title
                                        className="category-name">{mealCategory.categoryName}</Typography.Title>
                                    <ul className="meals-list">
                                        {mealCategory.meals.map(meal => {
                                            return <li key={meal.id}
                                                       className="meal">{meal.name + " " + meal.price + "zł"}
                                                {this.isAdded(meal.id) ?
                                                    <Button className="meal-btn" onClick={() => this.removeMeal(meal)}
                                                            type="danger">Usuń</Button> :
                                                    <Button className="meal-btn" onClick={() => this.addMeal(meal)}
                    ...
                    <Select style={{width: 300}} placeholder="Wybierz miejsce dostawy"
                    ...
                    <Checkbox.Group style={{width: '100%'}}>
                        <Checkbox value={1} onChange={this.changeWeekdays}>Poniedziałek</Checkbox>
                        <Checkbox value={2} onChange={this.changeWeekdays}>Wtorek</Checkbox>
                        <Checkbox value={3} onChange={this.changeWeekdays}>Środa</Checkbox>
                        <Checkbox value={4} onChange={this.changeWeekdays}>Czwartek</Checkbox>
                        <Checkbox value={5} onChange={this.changeWeekdays}>Piątek</Checkbox>
                        <Checkbox value={6} onChange={this.changeWeekdays}>Sobota</Checkbox>
                        <Checkbox value={7} onChange={this.changeWeekdays}>Niedziela</Checkbox>
                    </Checkbox.Group>
                    <span>Wartość zamówienia: {price}zł</span>
                    <DatePicker defaultValue={date} onChange={this.setOrderDate}/>
                    <Button type="primary" onClick={this.createOrder}>Zamów</Button>
                </div>
            </div>
        );
    }
